...
import { MdOutlineCardGiftcard, MdOutlineHowToVote, MdOutlineLock, MdOutlineSettings, MdOutlineHome } from 'react-icons/md';
...
const SideMenu: React.FC<SideMenuProps> = ({ address, isOpen, isSSSLinked }) => {
  ...
  const menuItems = [
    ...
    { text: 'ガバナンス投票', path: '/governance', icon: <MdOutlineHowToVote />, requiresSSS: true },
    { text: '特別会員限定', path: '/limited', icon: <MdOutlineLock />, requiresSSS: true },
    ...(isManagerAccount ? [
      { text: 'DAO設定', path: '/dao', icon: <MdOutlineSettings />, requiresSSS: true },
      { text: '特典管理', path: '/reward', icon: <MdOutlineCardGiftcard />, requiresSSS: true },
      { text: 'ポイント管理', path: '/point', icon: <BiCoinStack />, requiresSSS: true },
    ] : [])
  ];
  ...
  const menuLinkStyle = (isActive: boolean, isHovered: boolean, isDisabled: boolean) => ({
  ...
  return (
    <div style={sideMenuStyle}>
      <ul style={menuListStyle}>
        {menuItems.map((item) => {
          const isDisabled = (!isSSSLinked && item.requiresSSS) || 
