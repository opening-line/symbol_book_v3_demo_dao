...
export const getMemberMosaics = async (c: Context) => {
  try {
    ...
    const mosaics = await Promise.all(res.mosaics.map(async (mosaic: { id: string, amount: string }) => {
      const mosaicInfo = await getMosaicInfo(mosaic.id);
      return {
        id: mosaic.id,
        amount: convertToMosaicActualAmount(mosaic.amount, mosaicInfo.divisibility),
      };
    }));
